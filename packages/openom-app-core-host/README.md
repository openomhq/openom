...
| **APP-HOST-7** | Native DAG reconciliation verifies and atomically persists a newer anchor before activating its membership and epoch, while a cryptographically covered stale remote is preserved as a republish opportunity. | A rotated epoch must become usable without allowing a stale or incomparable server anchor to roll native custody backward. | `tests::a_dag_member_adopts_a_rotated_epoch_and_recovers_a_stale_remote` |
