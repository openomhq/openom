...
fn add(role: KeyringRole, seed: u8) -> MembershipAction<String, KeyringRole, openom_keyring_dag::Ed25519> {
...
fn dag_merges_non_conflicting_concurrent_edits_where_the_chain_would_serialize() {
    ...
    for e in [&ea, &eb] {
        assert!(has(e, 4) && has(e, 5), "the DAG merges both concurrent adds — neither is lost");
    }
...
fn dag_tallies_quorum_approvals_that_arrive_on_a_fork() {
    ...
    k.apply(sign(1, vec![], mid(1), 1, MembershipAction::Create { initial_members: cast.to_vec() }))
        .unwrap();
    let promote = MembershipAction::ChangeRole { member: mid(6), new_role: KeyringRole::CO_OWNER };
    // propose on the main line, then TWO approvals on concurrent branches off the proposal.
    k.apply(sign(2, vec![[1; 32]], mid(2), 2, MembershipAction::Propose { proposal_id: [7; 32], target: Box::new(promote) })).unwrap();
    k.apply(sign(3, vec![[2; 32]], mid(3), 3, MembershipAction::Approve { proposal_id: [7; 32] })).unwrap();
    k.apply(sign(4, vec![[2; 32]], mid(1), 1, MembershipAction::Approve { proposal_id: [7; 32] })).unwrap();
    // commit references both concurrent approvals in its causal past.
    k.apply(sign(5, vec![[3; 32], [4; 32]], mid(2), 2, MembershipAction::Commit { proposal_id: [7; 32] })).unwrap();

    assert_eq!(
        k.state().members.get(&mid(6)).filter(|m| m.is_active()).map(|m| m.role),
